import { Link } from "react-router-dom";
import { SidebarData } from "../data/SidebarData.js";

const Sidebar = () => {
  return (
    // sidebar container
    <div className="sidebar group w-[80px] h-auto mx-[10px]  z-300 absolute transition-all duration-400 ">
      {/* sidebar box */}
    <div className="sidebar-box h-[calc(100vh-100px)] border-1 border-transparent group-hover:w-[240px] group-hover:border-[#E4E7EC] custom-scroll w-full bg-white flex flex-col gap-6 pt-[36px] group-hover:pt[20px] pb-2 px-2 rounded-[12px] overflow-x-hidden overflow-y-auto transition-all duration-200">
        {/* profile box */}
        <div className="w-full flex flex-col justify-center items-center">
          <span
            className="w-full max-w-[50px] group-hover:maxw-[66px] h-auto aspect-square bg-[#D9D9D9] rounded-[50%] bg-center bg-no-repeat"
            style={{backgroundImage: "url(data:image/png;base64,)"
                , backgroundSize: 'cover'
            }}
          >
          </span>
          <p className="sidebar-username text-black font-[400] text-[14px]">{SidebarData.Profilebox.user_name}</p>
        </div>

        {/* main menu box */}
        <div className="main-menu-box w-full flex flex-col justify-center align-middle px-2">
            {/* <span className="w-full sidebarLink flex justify-center group-hover:justify-start align-middle gap-2 mb-2">
                <i className="text-[#525967] group-hover:hidden">{IconsData.hamburger}</i>
                <h1 className="sidebar-text uppercase text-[#525967] text-[12px] font-[500]">Main menu</h1>
            </span> */}
          <h1 className="sidebar-text uppercase text-[#525967] text-[12px] font-[500]">Main menu</h1>
          
          {SidebarData.Mainmenu.map((each) => (
            <Link key={each.icon+each.label} className="sidebarLink place-content-center transition-colors duration-105 text-[#525967] hover:text-white" to={each.link}>
              <span className="group/span flex justify-center group-hover:justify-start align-middle hover:bg-[#1E9AB0] hover:text-white hover:rounded-md flex-row p-2 gap-2">
                <i className="text-[#1E9AB0] group-hover/span:text-[#fff]">{each.icon}</i>
                <p className="sidebar-text capitalize">
                    {each.label}
                </p>
              </span>
            </Link>
          ))}
        </div>

        {/* Settings box */}
        <div className="px-2">
          <h1 className="sidebar-text uppercase text-[#525967] text-[12px] font-[500]">Settings</h1>

          {SidebarData.Settings.map((each) => (
            <Link key={each.icon+each.label} className="place-content-center text-[#525967] hover:text-white hover:shadow transition-colors duration-105" to={each.link}>
              <span className="sidebarLink group/span flex justify-center group-hover:justify-start align-middle hover:bg-[#1E9AB0] hover:text-white hover:rounded-md flex-row p-2 gap-2">
                <i className="text-[#1E9AB0] group-hover/span:text-[#fff]">{each.icon}</i>
                <p className="sidebar-text capitalize">
                    {each.label}
                </p>
              </span>
            </Link>
          ))}
        </div>
      </div>
    </div>
  );
}

export default Sidebar;
