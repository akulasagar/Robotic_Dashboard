...
export default function Robots() {
  ...
  const [activeRobotType, setActiveRobotType] = useState(RoboData1[0]?.type || "");
  const navigate = useNavigate();

  // Filter robots by type
  const filteredRobots = robots.filter((robot) => robot.type === activeRobotType);
            ...
            <div className="mx-2 grid grid-cols-4 w-full gap-2">
              {RoboData1.map((each) => (
                <div
                  ...
                  className={`h-[200px] max-w-[318px] w-full rounded-[14px] shadow-sm p-[20px] cursor-pointer relative ${activeRobotType === each.type ? "bg-[#1E9AB0] text-white" : "bg-white"}`}
                >
                  <img className="h-[75px] w-full max-w-[75px]" src={each.image} alt={each.label} />
                  ...
                  <div className="flex flex-row justify-between items-center mt-2">
                    ...
                    <p
                      className={`w-[30px] h-[30px] rounded-[50%] flex items-center justify-center ml-auto ${activeRobotType === each.type ? "bg-[#CBDF70] text-white" : "bg-[#E8F0E7] text-[#1B650E]"}`}
                    >
                      {each.count}
                    </p>
                  </div>
                    ...
                    <div className="flex flex-col gap-y-2 mt-10">
                      ...
                      <div className="flex flex-row items-center gap-x-3 w-full mt-2">
                        ...
                        <div className="relative w-full h-[5px] rounded-full bg-gray-200 overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-300 ${each.battery >= 75 ? "bg-green-500" : each.battery >= 35 ? "bg-yellow-400" : "bg-red-500"}`}
                            style={{ width: `${each.battery}%` }}
                          />
                        </div>
                        <span className="text-sm font-medium whitespace-nowrap">{each.battery}%</span>
                      </div>
